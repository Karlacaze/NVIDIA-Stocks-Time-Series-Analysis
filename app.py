...
def load_and_train(csv_bytes):
    ...
    model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=2e-4), loss='huber', metrics=['mae'])
    callbacks = [
    ...
    forecast_dates = pd.bdate_range(start=df['Date'].max() + pd.Timedelta(days=1), periods=30)
    ...
    metrics = {
        ...
        'total_params': model.count_params(), 'epochs_trained': len(history.history['loss']),
    }
    data_info = {
...
tab1, tab2, tab3, tab4, tab5 = st.tabs([
    "🔮  FORECAST", "📊  PERFORMANCE", "📉  TRAINING", "🧩  ARCHITECTURE", "🗄️  DATASET"
])
...
with tab1:
    ...
    with col_l:
        ...
        selected_date = st.selectbox("", options=valid_dates,
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=recent['Date'], y=recent['Adj Close'], mode='lines', name='Historical',
                             line=dict(color='#5e7bff', width=2)))
    fig.add_trace(go.Scatter(x=list(forecast_dates[:forecast_days]), y=forecast_prices_arr[:forecast_days],
                             mode='lines+markers', name='Forecast',
                             line=dict(color='#00ffb3', width=2.5),
                             marker=dict(size=4, color='#00ffb3')))
    fig.add_trace(go.Scatter(x=[selected_date], y=[sel_price], mode='markers', name='Selected',
                             marker=dict(size=13, color=price_col, symbol='star',
                                         line=dict(color='white', width=1.5))))
    fig.add_vline(x=str(df['Date'].max().date()), line=dict(color='#1e2340', dash='dash', width=1.5))
...
with tab2:
    ...
    fig2 = go.Figure()
    fig2.add_trace(go.Scatter(x=test_dates, y=prices_real_df, mode='lines', name='Real',
                              line=dict(color='#5e7bff', width=2)))
    fig2.add_trace(go.Scatter(x=test_dates, y=prices_pred_arr, mode='lines', name='Predicted',
                              line=dict(color='#ff6b35', width=2, dash='dash')))
    fig2.add_trace(go.Scatter(x=list(test_dates)+list(test_dates[::-1]),
                              y=list(prices_real_df)+list(prices_pred_arr[::-1]),
                              fill='toself', fillcolor='rgba(255,107,53,0.06)',
                              line=dict(color='rgba(0,0,0,0)'), showlegend=False))
    styled_layout(fig2, f"MAE ${metrics['mae']:.2f}  ·  RMSE ${metrics['rmse']:.2f}  ·  R² {metrics['r2']:.4f}  ·  Dir {metrics['dir_acc']:.1f}%", height=400)
...
with tab3:
    ...
    with col_a:
        fig_l = go.Figure()
        fig_l.add_trace(go.Scatter(x=ep, y=tl, mode='lines', name='Train', line=dict(color='#5e7bff', width=2)))
        fig_l.add_trace(go.Scatter(x=ep, y=vl, mode='lines', name='Val',   line=dict(color='#ff4466', width=2, dash='dash')))
    ...
    with col_b:
        fig_m = go.Figure()
        fig_m.add_trace(go.Scatter(x=ep, y=tm, mode='lines', name='Train MAE', line=dict(color='#ffd060', width=2)))
        fig_m.add_trace(go.Scatter(x=ep, y=vm, mode='lines', name='Val MAE',   line=dict(color='#ff6b35', width=2, dash='dash')))
    ...
    c1.metric("Epochs", metrics['epochs_trained'])
    ...
    total = metrics['train_size'] + metrics['val_size'] + metrics['test_size']
    fig_p = go.Figure(go.Pie(
        labels=['Train', 'Validation', 'Test'],
        values=[metrics['train_size'], metrics['val_size'], metrics['test_size']],
        hole=0.6, marker=dict(colors=['#5e7bff','#ffd060','#ff6b35']),
        textinfo='label+percent'))
    fig_p.update_layout(**DARK, height=280, margin=dict(t=30,b=20,l=20,r=20),
                        font=dict(family='Syne', color='#e0e4ff'),
...
with tab4:
    ...
    with col_r:
        ...
        params = {
            ...
            "LR Scheduler":     "ReduceOnPlateau ×0.4",
...
with tab5:
    ...
    with col_a:
        ...
        fig_hist.add_trace(go.Histogram(x=df['Adj Close'], nbinsx=50,
                                        marker_color='#5e7bff', opacity=0.8, name='Adj Close'))
    ...
    with col_b:
        fig_vol = go.Figure()
        fig_vol.add_trace(go.Scatter(x=df['Date'], y=df['Volume']/1e6, mode='lines',
    ...
    fig_corr = go.Figure(go.Heatmap(
        ...
        textfont=dict(size=8), hovertemplate='%{x} · %{y}<br>r = %{z}<extra></extra>'
    ))
